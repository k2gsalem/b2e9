...
<div class="pt-5 {{ $class }}">
    <div class="relative">
        <input id="{{ $name }}" name="{{ $name }}" placeholder="{{ $label }}"
               class="peer w-full h-12 pl-3 pr-2 py-2 text-lg rounded-lg border border-gray-300 text-gray-500 placeholder-transparent focus:outline-none focus:ring-transparent focus:border-2 focus:border-secondary"
            {!! $attributes !!}
        />
        <label for="{{ $name }}" class="absolute left-3 -top-3.5 text-gray-600 text-base transition-all bg-white px-2 rounded-lg
