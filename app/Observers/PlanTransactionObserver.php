...
use App\Notifications\PlanInvoicePaid;
use Illuminate\Support\Str;

class PlanTransactionObserver
{
    public function creating(PlanTransaction $model)
    {
        $model->uuid = 'B2ES'.Str::upper(Str::random(6).Str::random(6));
    }

    public function updated(PlanTransaction $model)
    {
        if ($model->isDirty('paid_at')
            ...
            $model->user->notify(new PlanInvoicePaid($model));
        }
    }
}
